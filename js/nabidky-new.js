...
function syncPlochyToCalc() {
  // Priorita: souhrn z pudorysu (PS._projectSummary) → ručně zadané plochy → PS.rooms
  let totalArea = 0;
  if (window.PS?._projectSummary?.m2Podlaha > 0) {
    totalArea = window.PS._projectSummary.m2Podlaha;
  } else {
    totalArea = window.currentPudorysData.ploch.reduce((s, x) =>
      s + ((x.w * x.h) / 10000 - (x.otvory || []).reduce((oo, o) => oo + ((o.w * o.h) / 10000), 0)), 0);
    if (totalArea === 0) {
      const psRooms = PS?.rooms?.filter(r => r.points?.length >= 3) || [];
